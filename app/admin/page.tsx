  ...
  try {
                        ...
                        orderRows.map((order) => (
                          <tr key={order.id} className="border-t border-white/5 hover:bg-white/5">
                            ...
                            <td className="px-6 py-4">
                              <div className="text-white">{(order.profiles as any)?.full_name || 'Nieznany'}</div>
                              <div className="text-xs">{(order.profiles as any)?.email}</div>
                            </td>
    ...
    return (
      <div className="p-8 max-w-3xl mx-auto mt-24 bg-red-500/10 border border-red-500/30 rounded-2xl text-red-400">
        ...
        <pre className="text-sm overflow-auto p-4 bg-black/50 rounded-lg">{error?.message || String(error)}</pre>
        <pre className="text-xs overflow-auto p-4 mt-2 bg-black/50 rounded-lg text-gray-400">{error?.stack}</pre>
      </div>
    )
  }
}
